...
const editButton =
    document.getElementById("edit") as HTMLButtonElement;
const saveButton =
    document.getElementById("save") as HTMLButtonElement;
const deleteButton =
    document.getElementById("delete") as HTMLButtonElement;
const memoTitle =
    document.getElementById("memoTitle") as HTMLInputElement;
const memoBody =
    document.getElementById("memoBody") as HTMLTextAreaElement;
const previewBody =
    document.getElementById("previewBody") as HTMLDivElement;
const downloadLink =
...
function newMemoElement(memo: Memo): HTMLDivElement {
    ...
    div.classList.add('w-full', 'p-sum');
...
function setHiddenButton(
    button: HTMLButtonElement, isHidden: boolean) {
    if (isHidden) {
...
function setEditMode(editMode: boolean) {
    if (editMode) {
        ...
        // 編集モードの時はtextareaを表示し、プレヴュー用を非表示にする
        ...
        // 表示モード時はtextareaを非表示にし、プレヴュー用を表示する
